...
// LOGIKA KUNCI KURSI BERDASARKAN HARI YANG SAMA
function generateSeats() {
  ...
  // Kumpulkan semua kursi yang sudah dipesan HANYA pada hari yang sama
  let bookedSeatsForThisDay = [];
  bookings.forEach(b => {
    if (b && b.day === currentDay && b.seats && Array.isArray(b.seats)) {
      bookedSeatsForThisDay = bookedSeatsForThisDay.concat(b.seats);
    }
  });
  ...
  rows.forEach(row => {
    for (let i = 1; i <= cols; i++) {
      ...
      // Jika nomor kursi sudah ada pesanan di hari yang sama, kunci langsung (MERAH)
      if (bookedSeatsForThisDay.includes(seatId)) {
